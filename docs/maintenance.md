...
- 主学习分为 `quick`（快速认型）和 `deep`（深度掌握）两套独立状态，存于 `state.plans`；旧版 `state.srs` 在启动时迁移至 `quick`，禁止因界面更新清空进度。
- 快速认型每天从全部产品随机抽 20 个，深度掌握每天从 `state.plans.deep.selectedIds` 产品池随机抽 5 个；两者按独立复习梯度和负荷阈值生成每日批次。
- 两套学习卡片均保留不认识/模糊/认识/掌握；专项学习才仅用上一个/下一个。
- 快速答题考分类和类型；深度答题增加性能、成分、外观、固含量、应用与 pH（存在 pH 指标时）。错题和答题进度按方案隔离。
- 专项学习：项目自选产品，每天学全部项目产品；专项答题按“项目 id + 日期”种子生成，
  选择题（类型/主要成分/外观/固含量，每产品 1 题）在前，类型判断题约一半产品数在后（比例约 2:1，错误项取相近类型），
  同天稳定、隔天不同，不进错题本；答完按正确率显示得分，项目详情可查看近 7 天成绩；
  项目可完成归档、重新开启、删除。
